...
import { fetchReceptionistFollowUps, fetchReceptionistPatientDetails } from '../../../features/receptionist/receptionistThunks';
import { 
  Calendar, 
  User, 
  Activity, 
...
const FollowUp = () => {
  ...
  const { followUps, loading, error } = useSelector(state => state.receptionist);
  const { patientDetails } = useSelector(state => state.receptionist);

  useEffect(() => {
    dispatch(fetchReceptionistFollowUps());
  }, [dispatch]);
