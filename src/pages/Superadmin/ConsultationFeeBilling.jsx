...
const SuperadminConsultationFeeBilling = () => {
  ...
  // Set known centers immediately
  useEffect(() => {
    const knownCenters = [
      { _id: 'center1', name: 'Chanre Hospital', code: '2345', location: 'Bengaluru' },
      { _id: 'center2', name: 'ChanRe Veena Rheumotology and Immunology Center And Research', code: '0002', location: 'Bengaluru East' },
      { _id: 'center3', name: 'Test Center', code: '12345', location: 'test' },
      { _id: 'center4', name: 'Apollo', code: '0993', location: 'Bengaluru' }
    ];
    console.log('Setting known centers:', knownCenters);
    setCenters(knownCenters);
  }, []);

  useEffect(() => {
    filterTransactions();
  }, [allTransactions, searchTerm, statusFilter, typeFilter, centerFilter, centers]);

  // Create fallback centers from transactions if centers API fails
  useEffect(() => {
    if (allTransactions.length > 0 && centers.length === 0) {
      console.log('🔄 Creating fallback centers from transactions...');
      const centerMap = new Map();
      
      allTransactions.forEach(transaction => {
        console.log('Processing transaction for center data:', {
          patientName: transaction.patientName,
          centerId: transaction.centerId,
          centerName: transaction.centerName,
          centerCode: transaction.centerCode,
          centerIdType: typeof transaction.centerId
        });
        
        // Try multiple ways to get center information
        let centerId = null;
        let centerName = null;
        let centerCode = null;
        
        // Method 1: Direct centerId object
        if (transaction.centerId && typeof transaction.centerId === 'object') {
          centerId = transaction.centerId._id;
          centerName = transaction.centerId.name;
          centerCode = transaction.centerId.code;
        }
        // Method 2: Direct centerId string
        else if (transaction.centerId && typeof transaction.centerId === 'string') {
          centerId = transaction.centerId;
          centerName = transaction.centerName;
          centerCode = transaction.centerCode;
        }
        // Method 3: Use transaction centerName directly
        else if (transaction.centerName) {
          centerId = transaction.centerName.toLowerCase().replace(/\s+/g, '_'); // Create ID from name
          centerName = transaction.centerName;
          centerCode = transaction.centerCode || 'N/A';
        }
        
        if (centerId && centerName && !centerMap.has(centerId)) {
          centerMap.set(centerId, {
            _id: centerId,
            name: centerName,
            code: centerCode || 'N/A'
          });
          console.log('Added center to map:', { _id: centerId, name: centerName, code: centerCode });
        }
      });
      
      const fallbackCenters = Array.from(centerMap.values());
      if (fallbackCenters.length > 0) {
        console.log('📊 Setting fallback centers:', fallbackCenters);
        setCenters(fallbackCenters);
      } else {
        console.log('⚠️ No centers found in transactions');
      }
    }
  }, [allTransactions, centers.length]);
  ...
  const clearReassignmentInfo = () => {
    ...
    navigate('/dashboard/superadmin/consultation-fee-billing', { replace: true });
  };
  ...
  const fetchAllTransactions = async () => {
    try {
      ...
      // Fetch all patients with both regular billing and reassignment billing
      ...
      if (response.data.success && response.data.patients) {
        ...
        patients.forEach(patient => {
          console.log('Processing patient:', {
            ...
            centerIdIsObject: typeof patient.centerId === 'object',
            centerIdFull: patient.centerId,
            patientFullData: patient
          });
          ...
          if (patient.billing && patient.billing.length > 0) {
            patient.billing.forEach((bill, index) => {
              transactions.push({
                ...
                centerId: patient.centerId,
                centerName: patient.centerId?.name || null,
                centerCode: patient.centerId?.code || null,
        ...
        console.log('Sample transaction data:', transactions.slice(0, 2).map(t => ({
          patientName: t.patientName,
          centerId: t.centerId,
          centerName: t.centerName,
          centerCode: t.centerCode,
          centerIdType: typeof t.centerId
        })));
  ...
  const filterTransactions = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(transaction =>
        ...
        transaction.centerName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        transaction.centerCode?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    if (centerFilter !== 'all') {
      ...
      console.log('Available centers:', centers.map(c => ({ id: c._id, name: c.name })));
      const beforeFilter = filtered.length;
      
      filtered = filtered.filter(transaction => {
        // Handle different center ID formats - try multiple approaches
        let transactionCenterId = null;
        
        // Try different ways to get the center ID
        if (transaction.centerId) {
          if (typeof transaction.centerId === 'object') {
            transactionCenterId = transaction.centerId._id;
          } else {
            transactionCenterId = transaction.centerId;
          }
        }
        
        const filterCenterId = centerFilter;
        
        // Also check if we can match by center name
        const transactionCenterName = transaction.centerName;
        const selectedCenter = centers.find(c => c._id === filterCenterId);
        const selectedCenterName = selectedCenter?.name;
        
        const matches = transactionCenterId === filterCenterId || 
                       transactionCenterName === selectedCenterName ||
                       transaction.centerId === filterCenterId;
        
        console.log('Center filter check:', {
          patientName: transaction.patientName,
          transactionCenterId,
          ...
          matches,
          centerIdType: typeof transaction.centerId,
          centerIdValue: transaction.centerId
        });
        
        return matches;
      });
      
      console.log('Center filter results:', {
        beforeFilter,
        afterFilter: filtered.length,
        selectedCenter: centers.find(c => c._id === centerFilter)
      });
      
      // If no results found, let's see what we have
      if (filtered.length === 0 && beforeFilter > 0) {
        console.log('⚠️ No transactions found for center filter. Showing sample transactions:');
        console.log('First 3 transactions:', allTransactions.slice(0, 3).map(t => ({
          patientName: t.patientName,
          centerId: t.centerId,
          centerName: t.centerName,
          centerIdType: typeof t.centerId,
          centerIdValue: t.centerId
        })));
      }
    }
  ...
  const getStats = () => {
    ...
    const uniqueCenters = [...new Set(allTransactions.map(t => t.centerId?._id || t.centerId).filter(Boolean))].length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-emerald-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-600 text-xs font-medium">Revenue</p>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-indigo-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-600 text-xs font-medium">Centers</p>
                <p className="text-lg font-bold text-slate-800">{stats.uniqueCenters}</p>
              </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search by patient name, email, phone, UH ID, doctor, center, or invoice number..."
                    value={searchTerm}
                  ...
                  >
                    ...
                    {centers && centers.length > 0 ? (
                      centers.map((center, index) => (
                        <option key={center._id || index} value={center._id}>
                            {center.name || 'Unknown Center'} ({center.code || 'No Code'})
                      </option>
                      ))
                    ) : (
                      <option value="loading" disabled>Loading centers...</option>
                    )}
                  </select>
                </div>
              </div>
          ...
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-slate-600 text-sm mb-4">
                {searchTerm || statusFilter !== 'all' || typeFilter !== 'all' || centerFilter !== 'all'
            ...
            <table className="w-full min-w-[800px]">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Center</th>
                  <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Transaction Type</th>
                  <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Amount & Status</th>
                  <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Payment Info</th>
                ...
                {currentTransactions.map((transaction) => {
                  ...
                  return (
                    <tr key={transaction.id} className={`hover:bg-slate-50 ${transaction.isReassigned ? 'bg-orange-50 border-l-4 border-orange-400' : ''}`}>
                      ...
                      <td className="px-3 sm:px-4 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-900">{transaction.centerName || 'N/A'}</div>
                        <div className="text-sm text-slate-500">{transaction.centerCode || 'N/A'}</div>
                      </td>
                      <td className="px-3 sm:px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
